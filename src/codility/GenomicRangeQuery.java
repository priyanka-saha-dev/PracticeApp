...
public class GenomicRangeQuery {
    ...
    private static int[] solution(String S, int[] P, int[] Q) {
        int M = P.length;//OR Q.length
        int[] answer = new int[M];

        Map<String, Integer> nucleotides = Map.of("A", 1, "C", 2, "G", 3, "T", 4);
        ...
        for (int N = 0; N < M; N++) {
            ...
            String part = "";
            if(posP != posQ) {
                part = S.substring(Math.min(posP, posQ), Math.max(posP, posQ));
            } else {
                part = S.substring(posP, posQ+1);
            }

            Optional<Integer> minimalImpactFactor = Arrays.stream(part.chars().toArray())
                    .mapToObj(Character::toString)
                    .map(nucleotides::get)
                    .filter(Objects::nonNull)
                    .min(Integer::compareTo);

            answer[N] = minimalImpactFactor.orElse(0);
