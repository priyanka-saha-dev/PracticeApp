...
public class Fish {
    ...
    public static void main(String[] args) {
        int answer = solution(new int[]{4, 3, 2, 5, 1}, new int[]{0, 1, 0, 0, 0});
//        int answer = solution(new int[]{4, 3, 5, 2, 1}, new int[]{0, 1, 0, 0, 0});
...
//        Arrays.stream(answer).forEach(System.out::println);
    ...
    private static int solution(int[] A, int[] B) {
        ...
        if(Arrays.stream(B).allMatch(e -> e == 1) || Arrays.stream(B).allMatch(e -> e == 0)) return A.length;
//        //s1 - Find the upstream counter in B
//        //s4 - repeat s2 & s3 for all upstream counter
//        for (int i = 0; i < B.length; i++) {
//            if (B[i] == 1) {
//
//                //s2 - Find the fish swimming upstream in A
//                int fish = A[i];
//
//                //s3 - Find all fishes greater than s2
//                List<Integer> strongerFishes = Arrays.stream(A).boxed().filter(e -> e > fish).toList();
//
//                answer += strongerFishes.size();
//            }
//        }
...
//                }
        ...
        //s3 - Find all fishes greater than s2 && higher positions
